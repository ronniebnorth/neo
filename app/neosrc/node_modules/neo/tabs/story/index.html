    ...
    <script>
      class StoryEditor extends PhElement {
        ...
        connectedCallback() {
          ...
          $(this).on('mousedown mousemove mouseup', this._handleMouseEvent);
        ...
        _handleMouseEvent(e) {
          var range = this.doc.rangeFromPoint(e.clientX, e.clientY);

          if (!range) {
            return false;
          }

          var cursor = this.doc.cursor;

          switch (e.originalEvent.type) {
            case 'mousedown': {
              range.insertNode(cursor.parentNode.removeChild(cursor));

              var anchor = this.doc.anchor || document.createElement('nui-anchor');

              cursor.parentNode.moveBefore(anchor, cursor);
            } break;
            ...
            case 'mouseup': {
              if (this.doc.anchor && !this.doc.selectionRange) {
